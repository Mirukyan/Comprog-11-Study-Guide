...
#include <stdlib.h>
#include <string.h>
#include <unistd.h> // Standard POSIX library for usleep() on Android/Linux

// Color definitions (ANSI Escape Codes work natively on CxxDroid)
...
// Portable screen clear function for Android/Linux terminal environments
void clearScreen(void) {
    printf("\033[H\033[2J");
    fflush(stdout);
}

// Portable typewriter animation function using POSIX usleep (milliseconds * 1000)
void typeWriter(const char* text, int msDelay) {
    for (int i = 0; text[i] != '\0'; i++) {
        putchar(text[i]);
        fflush(stdout);
        usleep(msDelay * 1000); // usleep takes microseconds
    }
}

// Visual feedback trigger for mobile environments replacing Windows Beep
void playSound(int isCorrect) {
    if (isCorrect) {
        printf("%s[♪ Correct!]%s ", COLOR_TERM, COLOR_RESET);
    } else {
        printf("%s[X Incorrect!]%s ", COLOR_RED, COLOR_RESET);
    }
    fflush(stdout);
}
...
// Interactive Exercise: Fill-in-the-Blank Code Practice
...
// Interactive Exercise: Output Challenge
...
// Option 5: Comprehensive Practice
void showComprehensivePractice(void) {
    ...
    if (percentage >= 80.0) {
        hasCompletedAllTopics = 1;
        playSound(1);
        printf("%sCONGRATULATIONS! You passed with %.1f%% and unlocked the Finale Sandbox!%s\n", COLOR_TERM, COLOR_RESET, percentage);
    } else {
        playSound(0);
        printf("%sFAILED (Below 80%% passing threshold)%s\n", COLOR_RED, COLOR_RESET);
        printf("You need at least 5/6 (80%%) correct to unlock the Finale Sandbox.\n");
        printf("Please review the modules and try again!\n");
    }
}

// Finale Option: Code Sandbox
        ...
        case 1:
            ...
            if (fgets(userCode, sizeof(userCode), stdin) != NULL) {
                ...
                if (strstr(userCode, "printf") && strstr(userCode, "C Programming") && strstr(userCode, ";")) {
                    playSound(1);
                    printf("\n%sCODE APPROVED!%s Excellent syntax. You included printf, string arguments, and the required semicolon.\n\n", COLOR_TERM, COLOR_RESET);
                } else {
                    playSound(0);
                    printf("\n%sCODE REVIEW FAILED%s\n", COLOR_RED, COLOR_RESET);
                    printf("--------------------------------------------------\n");
        ...
        case 2:
            ...
            if (fgets(userCode, sizeof(userCode), stdin) != NULL) {
                ...
                if (strstr(userCode, "for") && strstr(userCode, "i = 0") && strstr(userCode, "i < 10") && strstr(userCode, "i++")) {
                    playSound(1);
                    printf("\n%sCODE APPROVED!%s Perfect counter-controlled loop setup.\n\n", COLOR_TERM, COLOR_RESET);
                } else {
                    playSound(0);
                    printf("\n%sCODE REVIEW FAILED%s\n", COLOR_RED, COLOR_RESET);
                    printf("--------------------------------------------------\n");
        ...
        case 3:
            ...
            if (fgets(userCode, sizeof(userCode), stdin) != NULL) {
                ...
                if (strstr(userCode, "return") && (strstr(userCode, "x * 2") || strstr(userCode, "x*2") || strstr(userCode, "2 * x")) && strstr(userCode, ";")) {
                    playSound(1);
                    printf("\n%sCODE APPROVED!%s Excellent return logic.\n\n", COLOR_TERM, COLOR_RESET);
                } else {
                    playSound(0);
                    printf("\n%sCODE REVIEW FAILED%s\n", COLOR_RED, COLOR_RESET);
                    printf("--------------------------------------------------\n");
        ...
        case 4:
            ...
            if (fgets(userCode, sizeof(userCode), stdin) != NULL) {
                ...
                if (strstr(userCode, "scores[1]")) {
                    playSound(1);
                    printf("\n%sCODE APPROVED!%s Correct zero-based index targeting!\n\n", COLOR_TERM, COLOR_RESET);
                } else {
                    playSound(0);
                    printf("\n%sCODE REVIEW FAILED%s\n", COLOR_RED, COLOR_RESET);
                    printf("--------------------------------------------------\n");
...
void showCredits(void) {
    ...
    typeWriter("  * Version:      v1.0.0 (Mobile POSIX Edition)\n", 12);
    typeWriter("  * Platform:     CxxDroid / POSIX Compliant\n\n", 12);
...
int main(void) {
    ...
    while (1) {
        clearScreen();
        printf("%s==================================================%s\n", COLOR_HEADER, COLOR_RESET);
        printf("%s   INTERACTIVE C STUDY GUIDE v1.0.0 (MOBILE)      %s\n", COLOR_HEADER, COLOR_RESET);
        printf("%s==================================================%s\n", COLOR_HEADER, COLOR_RESET);
